...
function getFrom(): string {
  return (
    ...
    "Coppedskins <noreply@coppedskins.com>"
  );
}
...
function getSiteUrl(): string {
  const raw = process.env.NEXT_PUBLIC_SITE_URL || "https://coppedskins.com";
  return raw.replace(/\/+$/, "");
}
  ...
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <div style="text-align:center;margin-bottom:24px;">
      <a href="${getSiteUrl()}" style="text-decoration:none;">
        <span style="font-size:24px;font-weight:900;color:${BRAND_COLOR};letter-spacing:-0.03em;">Coppedskins</span>
      </a>
    </div>
    ...
    <div style="text-align:center;margin-top:24px;font-size:12px;color:#999;line-height:1.6;">
      <p style="margin:0 0 4px;">&copy; ${new Date().getFullYear()} Coppedskins. All rights reserved.</p>
...
export async function sendWelcomeEmail(email: string, name?: string | null): Promise<boolean> {
  ...
  return send({
    to: email,
    subject: "Welcome to Coppedskins!",
    html: emailWrapper(
      `
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:${TEXT_COLOR};">${greeting}, welcome to Coppedskins!</h1>
    ...
    `,
      { preheader: `${greeting}! Your Coppedskins account is ready.` },
    ),
  });
}
  ...
  return send({
    ...
    html: emailWrapper(
      ...
      <p style="color:#999;font-size:12px;margin:24px 0 0;line-height:1.6;text-align:center;">
        ...
        For any questions, reply to this email or contact <a href="mailto:info@coppedskins.com" style="color:${BRAND_COLOR};">info@coppedskins.com</a>.
      </p>
    `,
...
export async function sendPasswordResetEmail(email: string, resetUrl: string, name?: string | null): Promise<boolean> {
  ...
  return send({
    to: email,
    subject: "Reset your Coppedskins password",
    html: emailWrapper(
      `
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:${TEXT_COLOR};">Reset your password</h1>
      <p style="color:${MUTED_COLOR};line-height:1.6;margin:0 0 16px;">
        ${greeting}, we received a request to reset the password on your Coppedskins account.
    ...
    `,
      { preheader: "Reset your Coppedskins password — link expires in 1 hour." },
    ),
  });
}
...
export async function sendContactAutoReplyEmail(submission: ContactSubmission): Promise<boolean> {
  return send({
    to: submission.email,
    subject: "We received your message — Coppedskins",
    html: emailWrapper(
      `
