    ...
    <div className="mt-8 flex flex-col gap-3">
      {orders.map((o) => {
        ...
        return (
          <div
            key={o.id}
            className={`relative flex flex-col gap-3 overflow-hidden border bg-[color:var(--color-bg-elevated)] px-4 py-4 sm:flex-row sm:items-center sm:gap-5 sm:px-5 ${
              highlighted
                ? "border-[color:var(--color-text)] shadow-[var(--shadow-md)]"
                : "border-[color:var(--color-line)]"
            }`}
            style={o.rarityColor ? ({ ["--rarity" as string]: o.rarityColor }) : undefined}
          >
            {o.rarityColor ? (
              <span aria-hidden className="rarity-spine absolute inset-y-0 left-0 w-[3px]" />
            ) : null}

            <div className="relative h-16 w-24 shrink-0 overflow-hidden bg-[color:var(--color-bg-secondary)]">
              {o.imageUrl ? (
                <Image
                  src={o.imageUrl}
            ...
            <div className="min-w-0 flex-1">
              <div className="poster truncate text-[15px] text-[color:var(--color-text)]">
                {o.marketHashName}
              </div>
              <div className="readout mt-1 text-[11px] text-[color:var(--color-text-tertiary)]">
              ...
              {o.status === "sent" && o.senderOfferId ? (
                <a
                  ...
                  className="mt-2 inline-flex items-center gap-1.5 font-mono text-[11px] font-bold uppercase tracking-[0.12em] text-[color:var(--color-primary-strong)] hover:underline"
                >
                  Open trade offer
              ...
              {(o.status === "failed" || o.status === "refund_pending" || o.status === "rolled_back") ? (
                <div className="mt-2 border-l-[3px] border-[color:var(--color-danger)] bg-[color:var(--color-danger-light)] px-2.5 py-1.5 text-xs text-[color:var(--color-danger)]">
                  {o.status === "failed"
            ...
            <div className="flex items-center justify-between gap-4 sm:flex-col sm:items-end sm:gap-2">
              ...
              <span className="poster tnum whitespace-nowrap text-[20px] leading-none text-[color:var(--color-text)]">
                {fmt(o.price, "USD")}
              </span>
            </div>
          </div>
